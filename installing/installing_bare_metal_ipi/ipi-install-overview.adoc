...
1. Start with xref:../architecture/architecture.adoc#architecture-overview-architecture[Architecture].
2. xref:../security/container_security/security-understanding.adoc[Security and compliance]. 
This first xref is ok because it is an assembly. The second xref is not OK because it does not have an anchor ID.
