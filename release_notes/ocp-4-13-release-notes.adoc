...
A graceful node shutdown delays forcible eviction of pods during a node shutdown. You can configure the kublet to enable graceful node shutdowns to ensure that pods running critical workloads are not interrupted.

To configure graceful node shutdowns, specify a termination grace period for regular and critical pods. A termination grace period ensures that the pod shuts down in an orderly manner.
For example, if an operator that flashes network card firmware is shut down in a disorderly fashion, hardware can be damaged.
You can also add priority classes to pods to specify the order of shut down.
